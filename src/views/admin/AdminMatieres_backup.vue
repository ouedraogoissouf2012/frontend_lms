<template>
  <DashboardLayout>
    <div class="matieres-container">
      ...
      <!-- Matieres Table -->
      <div v-else-if="filteredMatieres.length > 0" class="table-card">
        <div class="table-wrapper">
          <table class="matieres-table-main">
            <thead>
              <tr>
                <th></th>
                <th>Matière</th>
            ...
            <tbody>
              <tr v-for="matiere in filteredMatieres" :key="matiere.id" class="table-row">
                <td><div class="matiere-color" :style="{ backgroundColor: matiere.couleur || '#6366f1' }"></div></td>
                <td class="col-matiere">
                  <div class="matiere-info">
                    <span class="matiere-name">{{ matiere.nom }}</span>
                    <span v-if="matiere.description" class="matiere-desc">{{ matiere.description.substring(0, 50) }}{{ matiere.description.length > 50 ? '...' : '' }}</span>
                  </div>
                </td>
                <td><span class="code-badge">{{ matiere.code || '-' }}</span></td>
                <td class="col-filieres">
                  <div class="badges-container">
                    <span v-for="(filiere, idx) in getMatiereFilieres(matiere)" :key="idx" class="filiere-badge" :title="filiere">{{ filiere }}</span>
                    <span v-if="getMatiereFilieres(matiere).length === 0" class="no-data">-</span>
                  </div>
                </td>
                <td class="col-niveaux">
                  <div class="badges-container">
                    <span v-for="(niveau, idx) in getMatiereNiveaux(matiere)" :key="idx" class="niveau-badge" :title="niveau">{{ niveau }}</span>
                    <span v-if="getMatiereNiveaux(matiere).length === 0" class="no-data">-</span>
                  </div>
                </td>
                <td class="col-center"><span class="coef-value">{{ matiere.coefficient || '-' }}</span></td>
                <td class="col-center"><span class="hours-badge">{{ matiere.heures_total || 0 }}h</span></td>
                <td class="col-center"><span class="seances-badge">{{ matiere.nb_seances_programmees || 0 }}</span></td>
                <td class="col-actions">
                  <button @click="viewMatiereDetails(matiere)" class="action-btn" title="Voir détails complets"><EyeIcon class="w-4 h-4" /></button>
                </td>
              </tr>
            </tbody>
          </table>
...
.matieres-table-main thead {
  background: var(--hover-bg);
}

.matieres-table-main th {
  padding: 0.75rem;
  text-align: left;
  font-weight: 600;
  color: var(--text-primary);
...
.matieres-table-main tbody tr {
  border-bottom: 1px solid var(--border-color);
  transition: background 0.2s;
}

.matieres-table-main tbody tr:hover {
  background: var(--hover-bg);
}

.matieres-table-main td {
  padding: 0.75rem;
  color: var(--text-secondary);
}
