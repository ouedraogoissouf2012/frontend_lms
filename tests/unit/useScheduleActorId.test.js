...
describe('useScheduleActorId (#330)', () => {
  it('uses local id when klassci_etudiant_id is absent', () => {
    state.user = { id: 42 }
    expect(useScheduleActorId('student').actorId.value).toBe(42)
  })

  it('prefers klassci_etudiant_id for a student', () => {
    state.user = { id: 42, klassci_etudiant_id: 7 }
    expect(useScheduleActorId('student').actorId.value).toBe(7)
  })

  it('uses local id when klassci_id is absent for a teacher', () => {
    state.user = { id: 9 }
    expect(useScheduleActorId('teacher').actorId.value).toBe(9)
  })

  it('returns null without a user', () => {
    state.user = null
    expect(useScheduleActorId('student').actorId.value).toBeNull()
  })
})
